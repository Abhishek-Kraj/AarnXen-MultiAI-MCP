...
@pytest.mark.asyncio
async def test_failures_trip_to_open():
...
async def test_multiple_providers_independent():
    ...
    await cb.record_failure("gemini")
    await cb.record_failure("gemini")
    await cb.record_failure("ollama")
...
@pytest.mark.asyncio
async def test_sliding_window_clears_old_failures():
...
@pytest.mark.asyncio
async def test_get_all_status_dashboard():
    cb = CircuitBreaker(failure_threshold=2)
    await cb.record_success("gemini")
    await cb.record_failure("ollama")
    await cb.record_failure("ollama")
...
@pytest.mark.asyncio
async def test_open_status_includes_retry_after():
    cb = CircuitBreaker(failure_threshold=2, cooldown_seconds=30)
    await cb.record_failure("gemini")
    await cb.record_failure("gemini")
    status = cb.get_status("gemini")
